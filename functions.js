...
document.addEventListener("click", function(e) {
 ...
 function buildGainersLosersHtml(allCompanies, currentCompanyId, segmentName) {
    ...
    function filterMarketTrends() {
      ...
      function openDetailsPanel(clickedEl, insertAfterEl, rowData) {
        const clickedIndex = Number(clickedEl.getAttribute("data-pla-index"));
      
        // If a panel is already open, update its content immediately:
        if (currentlyOpenPanel) {
          // If the same block is clicked, then toggle-close the panel.
          if (currentlySelectedIndex !== null && clickedIndex === currentlySelectedIndex) {
            closeDetailsPanel();
          ...
          // Otherwise, update the panel’s content:
          // Remove "selected" class from any other ad-details and add to the clicked element.
          document.querySelectorAll('.ad-details.selected').forEach(el => el.classList.remove('selected'));
          clickedEl.classList.add('selected');
          window.currentRowData = rowData;
          currentlySelectedIndex = clickedIndex;
          
          let newInsertAfterEl;
          const resultsEl = document.getElementById("filteredResults");
          if (resultsEl.classList.contains("list-mode")) {
            newInsertAfterEl = clickedEl;
          } else {
            const rowEndIndex = getRowEndIndex(clickedIndex);
            newInsertAfterEl = document.querySelector('.ad-details[data-pla-index="' + rowEndIndex + '"]');
          }
          
          // Reposition the panel if needed:
          if (newInsertAfterEl && currentlyOpenPanel.previousElementSibling !== newInsertAfterEl) {
            currentlyOpenPanel.remove();
            newInsertAfterEl.insertAdjacentElement("afterend", currentlyOpenPanel);
          }
          // Immediately update the panel's content (bypassing the panelAnimating check)
          ReactDOM.render(
            React.createElement(window.DetailsPanel, {
              key: clickedIndex, // force a remount using a unique key
              rowData,
              start: getDataRange(rowData).start,
              end: getDataRange(rowData).end,
        ...
        // If no panel is open, first check panelAnimating:
        if (panelAnimating) return;
        panelAnimating = true;
      
        // Remove "selected" class from any other ad-details elements and mark this one.
        document.querySelectorAll('.ad-details.selected').forEach(el => el.classList.remove('selected'));
        clickedEl.classList.add('selected');
        currentlySelectedIndex = clickedIndex;
        window.currentRowData = rowData;
        ...
        const resultsEl = document.getElementById("filteredResults");
        if (resultsEl.classList.contains("list-mode") && insertAfterEl) {
          if (insertAfterEl.nextElementSibling &&
              insertAfterEl.nextElementSibling.classList.contains("pla-details-panel")) {
            insertAfterEl.nextElementSibling.remove();
          }
        }
      
        // Create a new details panel element.
        const panel = document.createElement("div");
        panel.className = "pla-details-panel";
        panel.style.opacity = "0";
        panel.style.transform = "translateY(-10px)";
        if (insertAfterEl) {
          insertAfterEl.insertAdjacentElement("afterend", panel);
        } else {
          document.getElementById("filteredResults").appendChild(panel);
        }
        ...
        // Render the React details panel into the newly created panel.
        ReactDOM.render(
          React.createElement(window.DetailsPanel, {
            key: clickedIndex,
            rowData,
...
// Try to restore myCompany from localStorage if it's cached
// Defer setting myCompany until selectedAccount is defined
window.myCompanyReady = new Promise((resolve) => {
  window._resolveMyCompany = resolve;
});

const currentSelector = (document.getElementById("selectedAccountText")?.textContent || "").trim();

if (currentSelector === "DEMO") {
  console.warn("[INIT] DEMO selected — forcing myCompany = 'Nike'");
  window.myCompany = "Nike";
  localStorage.setItem("my_company", "Nike");
} else {
  const stored = localStorage.getItem("my_company");
  if (stored) {
    console.log("[INIT] Restored myCompany from localStorage:", stored);
    window.myCompany = stored;
  } else {
    window.myCompany = "";
  }
}
    ...
    window.addEventListener("message", async (event) => {
        ...
        if (typeof event.data === "string") {
          ...
          else if (data.command === "loadToggleStates") {
            
            console.log("[DEBUG] set myCompany from parent message:", data.myCompany);
            // NEW: process the toggles array
            const togglesArray = data.data || [];
        
            // Store each toggle state in window.localEmbedToggles
            togglesArray.forEach(t => {
              window.localEmbedToggles[t.toggleId] = t.isChecked;
              const checkbox = document.getElementById(t.toggleId);
              if (checkbox) {
                checkbox.checked = t.isChecked;
              }
            });

            const currentSelectorText = (document.getElementById("selectedAccountText")?.textContent || "").trim();
            if (currentSelectorText === "DEMO") {
              console.warn("[DEMO MODE] Skipping overwrite of window.myCompany — keeping 'Nike'");
            } else {
              console.log("[SET] window.myCompany from parent message:", data.myCompany);
              window.myCompany = data.myCompany || "";
              localStorage.setItem("my_company", window.myCompany);
            }            
            
            if (typeof window._resolveMyCompany === "function") {
              window._resolveMyCompany();  // ✅ Mark as ready
              window._resolveMyCompany = null; // prevent duplicate resolve
            }            
console.log("[DEBUG] set myCompany from parent message:", data.myCompany);
            // Apply the new toggle states to update the UI
            updateAdCards();
          }
          else if (data.command === "deliverTables") {
            console.log("[Embed] Received project data from parent:", data);
            const { projectNumber, processed, serpStats, marketTrends } = data;
          
            // Save active project number globally
            if (!window.filterState) window.filterState = {};
            window.filterState.activeProjectNumber = projectNumber;
            console.log(`[DEBUG] Set activeProjectNumber = ${projectNumber}`);
            ...
            console.log("[deliverTables] received projectNumber =", projectNumber);
console.log("[deliverTables] serpStats sample =", serpStats?.[0]);
          
if (Array.isArray(serpStats)) {
  window.companyStatsData = serpStats || [];

  if (Array.isArray(window.companyStatsData)) {
  window.companyStatsData.forEach(row => {
    if (row.project_number == null) {
      row.project_number = projectNumber;  // ✅ use from deliverTables
    }
  });
}

  if (Array.isArray(window.companyStatsData)) {
    window.companyStatsData.forEach((row, idx) => {
      if (row.source == null) {
        row.source = window.myCompany || "Unknown";
      }

      // ✅ Always show what we're trying to match
      console.log(`[Row ${idx}] Before patch — row.project_number =`, row.project_number, "dataPrefix =", window.dataPrefix);

      // 🧩 Infer project_number from window.dataPrefix
      if (row.project_number == null) {
        let projectNumber = null;

        // Attempt to extract from window.dataPrefix (e.g. acc1_pr2_ → 2)
        if (typeof window.dataPrefix === "string") {
          const match = window.dataPrefix.match(/pr(\d+)_/);
          if (match) {
            projectNumber = parseInt(match[1], 10);
            console.log(`[Row ${idx}] ✅ Inferred project_number from dataPrefix:`, projectNumber);
          } else {
            console.warn(`[Row ${idx}] ❌ Failed regex match on dataPrefix:`, window.dataPrefix);
          }
        }

        // Fallback: use projectNumber from the outer deliverTables scope if available
        if (!projectNumber && typeof window.filterState?.activeProjectNumber === "number") {
          projectNumber = window.filterState.activeProjectNumber;
          console.log(`[Row ${idx}] 🪂 Used fallback activeProjectNumber:`, projectNumber);
        }

        if (projectNumber) {
          row.project_number = projectNumber;
        } else {
          console.warn(`[Row ${idx}] 🚨 Could not set project_number. Row may be filtered out.`);
        }
      }
    });
  }

  console.groupCollapsed("[🛠️ Patched companyStatsData]");
  console.log("Sample row:", window.companyStatsData[0]);
  console.log("Total rows:", window.companyStatsData.length);
  console.log("Inferred prefix:", window.dataPrefix);
  console.groupEnd();

} else {
  window.companyStatsData = [];
  console.warn("[deliverTables] ❌ serpStats is not an array!");
}
            ...
            // Hide the working overlay if visible
            const workingOverlay = document.getElementById("workingOverlay");
            if (workingOverlay) {
              console.log("[DEBUG] Hiding #workingOverlay after fresh deliverTables");
              workingOverlay.style.display = "none";
            }
          }          
        }, false);      
