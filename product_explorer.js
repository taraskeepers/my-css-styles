...
// ADD THIS DEBUG SECTION at the beginning of renderProductExplorerTable function
// Place this right after line ~25 where you have:
// console.log("[renderProductExplorerTable] Starting to build product map table");

function debugAllRowsData() {
  console.log("=== DEBUGGING window.allRows DATA STRUCTURE ===");
  
  // Basic existence check
  console.log("1. Basic Data Check:");
  console.log("   - window.allRows exists:", !!window.allRows);
  console.log("   - window.allRows is array:", Array.isArray(window.allRows));
  console.log("   - window.allRows length:", window.allRows?.length || 0);
  console.log("   - typeof window.allRows:", typeof window.allRows);
  
  if (!window.allRows || !Array.isArray(window.allRows) || window.allRows.length === 0) {
    console.log("❌ No data in window.allRows - investigation needed");
    return;
  }
  
  // Sample record structure
  console.log("\n2. Sample Record Structure:");
  const firstRecord = window.allRows[0];
  console.log("   - First record keys:", Object.keys(firstRecord));
  console.log("   - First record sample:", {
    title: firstRecord.title,
    source: firstRecord.source,
    q: firstRecord.q,
    location_requested: firstRecord.location_requested,
    device: firstRecord.device,
    product_status: firstRecord.product_status,
    historical_data_length: firstRecord.historical_data?.length || 0
  });
  
  // Company analysis
  console.log("\n3. Company/Source Analysis:");
  const companyCounts = {};
  window.allRows.forEach(row => {
    const company = row.source || 'Unknown';
    companyCounts[company] = (companyCounts[company] || 0) + 1;
  });
  console.log("   - Companies found:", Object.keys(companyCounts));
  console.log("   - Records per company:", companyCounts);
  console.log("   - Current myCompany:", window.myCompany);
  ...
  window.allRows.forEach(row => {
    if (row.title) productTitles.add(row.title);
    
    const company = row.source || 'Unknown';
    if (!productsByCompany[company]) productsByCompany[company] = new Set();
    if (row.title) productsByCompany[company].add(row.title);
  });
  
  console.log("   - Total unique product titles:", productTitles.size);
  console.log("   - Products per company:");
  Object.keys(productsByCompany).forEach(company => {
    console.log(`     ${company}: ${productsByCompany[company].size} unique products`);
  });
  ...
  window.allRows.forEach(row => {
    if (row.q) searchTerms.add(row.q);
  });
  console.log("   - Unique search terms:", searchTerms.size);
  console.log("   - Sample search terms:", Array.from(searchTerms).slice(0, 5));
  
  // Location analysis
  console.log("\n6. Location Analysis:");
  const locations = new Set();
  window.allRows.forEach(row => {
    if (row.location_requested) locations.add(row.location_requested);
  });
  console.log("   - Unique locations:", locations.size);
  console.log("   - Sample locations:", Array.from(locations).slice(0, 5));
  ...
  console.log("   - Unique devices:", Array.from(devices));
  
  // Combination analysis for current company
  console.log("\n8. Current Company Data Analysis:");
  const myCompanyData = window.allRows.filter(row => 
    row.source && row.source.toLowerCase() === (window.myCompany || "").toLowerCase()
  );
  console.log("   - Records for current company:", myCompanyData.length);
  
  if (myCompanyData.length > 0) {
    const combinations = new Set();
    const productCombinations = {};
    
    myCompanyData.forEach(row => {
      const combo = `${row.q || 'N/A'}|${row.location_requested || 'N/A'}|${row.device || 'N/A'}`;
      combinations.add(combo);
      
      const product = row.title || 'Unknown Product';
      if (!productCombinations[product]) productCombinations[product] = new Set();
      productCombinations[product].add(combo);
    });
    
    console.log("   - Unique term+location+device combinations:", combinations.size);
    console.log("   - Sample combinations:", Array.from(combinations).slice(0, 3));
    
    console.log("   - Products and their combinations:");
    Object.keys(productCombinations).slice(0, 5).forEach(product => {
      console.log(`     "${product}": appears in ${productCombinations[product].size} combinations`);
    });
  }
  
  // Historical data analysis
  console.log("\n9. Historical Data Analysis:");
  let recordsWithHistData = 0;
  let totalHistoricalEntries = 0;
  let minHistEntries = Infinity;
  let maxHistEntries = 0;
  
  window.allRows.forEach(row => {
    if (row.historical_data && Array.isArray(row.historical_data) && row.historical_data.length > 0) {
      recordsWithHistData++;
      totalHistoricalEntries += row.historical_data.length;
      minHistEntries = Math.min(minHistEntries, row.historical_data.length);
      maxHistEntries = Math.max(maxHistEntries, row.historical_data.length);
    }
  });
  
  console.log("   - Records with historical data:", recordsWithHistData);
  console.log("   - Total historical entries:", totalHistoricalEntries);
  console.log("   - Min historical entries per record:", minHistEntries === Infinity ? 0 : minHistEntries);
  console.log("   - Max historical entries per record:", maxHistEntries);
  console.log("   - Avg historical entries per record:", recordsWithHistData > 0 ? (totalHistoricalEntries / recordsWithHistData).toFixed(1) : 0);
  
  // Sample historical data structure
  if (recordsWithHistData > 0) {
    const recordWithHist = window.allRows.find(row => 
      row.historical_data && Array.isArray(row.historical_data) && row.historical_data.length > 0
    );
    if (recordWithHist && recordWithHist.historical_data[0]) {
      console.log("   - Sample historical entry keys:", Object.keys(recordWithHist.historical_data[0]));
      console.log("   - Sample historical entry:", recordWithHist.historical_data[0]);
    }
  }
