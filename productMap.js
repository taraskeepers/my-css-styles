...
const allProductsForChart = [...sortedActiveProducts, ...sortedInactiveProducts];
              ...
              productCellDiv.querySelectorAll('.ad-details').forEach(adCard => {
                ...
                // Create a Chart.js-based replacement for PLAChart
window.PLAChartReplacement = function(props) {
  ...
  React.useEffect(() => {
    if (!containerRef.current || !rowData || !rowData.historical_data) return;
    ...
    const positionData = dateLabels.map(date => {
      const histItem = rowData.historical_data.find(item => 
        item.date && item.date.value === date
      );
      return histItem && histItem.avg_position ? parseFloat(histItem.avg_position) : null;
    });
    
    const visibilityData = dateLabels.map(date => {
      const histItem = rowData.historical_data.find(item => 
        item.date && item.date.value === date
      );
      return histItem && histItem.visibility ? parseFloat(histItem.visibility) * 100 : null;
    });
...
// Override PLAChart with our Chart.js version
window.PLAChart = window.PLAChartReplacement;
